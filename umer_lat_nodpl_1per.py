...
numCells = 1 # for matching the beam without the dipoles
...
top.a0       = 0.00147119094268995#beam.a0
top.b0       = 0.00307780016603408#beam.b0
#top.ap0      =-0.00854242568214239#beam.ap0
#top.bp0      = 0.0200773584700971#beam.bp0
top.ap0      = 0.0
top.bp0      = 0.0
# 6mA
...
for turn in range(turns):
    ...
    for i in range(numCells):
        ...
        #zloc+=DC_fodo_1
        addnewquad(zs=zloc, ze=zloc+Q_l_eff_D/2.0, db=+QD_current*params['QD'][2]*params['QD'][0])
...
fma()
hpxsqbar(color='blue')
hpysqbar(color='red',linetype='dash')
fma()
hpxpbar(color='blue')
hpypbar(color='red',linetype='dash')
fma()
hpxpsqbar(color='blue')
hpypsqbar(color='red',linetype='dash')
fma()
hpepsx(color='blue')
...
filename = sys.argv[0][:sys.argv[0].find(".py")]
filename = "data/" + filename

# --- save Poincare sections at each turn (x,y,ux,uy) for the beam
np.savetxt(filename+'_X.txt',x)
np.savetxt(filename+'_Y.txt',y)
np.savetxt(filename+'Xp.txt',xp)
np.savetxt(filename+'Yp.txt',yp)
# --- save centroid values in z, x_c, y_c
np.savetxt(filename+'_centroid_x_y.txt',np.array([top.hzbar[0,idx,0], top.hxbar[0,idx,0], top.hybar[0,idx,0]]).T)
np.savetxt(filename+'_centroid_xp_yp.txt',np.array([top.hzbar[0,idx,0], top.hxpbar[0,idx,0], top.hypbar[0,idx,0]]).T)
# --- save sigma matrix values sequentially
np.savetxt(filename + '_sigma_xx.txt',np.array([top.hzbar[0,idx,0],top.hxsqbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_xxp.txt',np.array([top.hzbar[0,idx,0],top.hxxpbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_xy.txt',np.array([top.hzbar[0,idx,0],top.hxybar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_xyp.txt',np.array([top.hzbar[0,idx,0],top.hxypbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_xpxp.txt',np.array([top.hzbar[0,idx,0],top.hxpsqbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_yxp.txt',np.array([top.hzbar[0,idx,0],top.hyxpbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_xpyp.txt',np.array([top.hzbar[0,idx,0],top.hxpypbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_yy.txt',np.array([top.hzbar[0,idx,0],top.hysqbar[0,idx,0]]).T)
np.savetxt(filename + '_sigma_yyp.txt',np.array([top.hzbar[0,idx,0],top.hyypbar[0,idx,0]]).T)
np.savetxt(FOLDER_LOC + '_sigma_ypyp.txt',np.array([top.hzbar[0,idx,0],top.hypsqbar[0,idx,0]]).T)
np.savetxt(filename + '_eps_x.txt',np.array([top.hzbar[0,idx,0],top.hepsx[0,idx,0]]).T)
np.savetxt(filename + '_eps_y.txt',np.array([top.hzbar[0,idx,0],top.hepsy[0,idx,0]]).T)
